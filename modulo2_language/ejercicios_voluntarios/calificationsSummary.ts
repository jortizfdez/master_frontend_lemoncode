...
const getAvg = (values: number[]): number => {
  return parseFloat((values.reduce((a,b) => a+b, 0) / values.length).toPrecision(3));
}

const summarizeStudent = (student: Student): StudentSummary=> {
   const result: StudentSummary = {
     name: student.name,
     highestCalification: getMax(student.califications),
     averageCalifications: getAvg(student.califications)
   };
...
const summarizeClassRoom = (studentList : Student[]) => {
  studentList.forEach(element => {
    console.log(summarizeStudent(element));
  });
